...
iOS asset catalog와 사용자가 제공한 네 PNG를 읽기 전용으로 대조했다. Android는 아래 사용자 제공 PNG byte를 `drawable-nodpi`에 그대로 보존한다. 재그림, crop, filter, 색 보정, 재인코딩을 하지 않으며 preview도 같은 원본 drawable을 사용한다. launcher에서 보이는 외곽 형태만 Android OS의 adaptive icon mask가 결정한다.
...
iOS asset catalog의 대응 이미지는 1024×1024 opaque지만 사용자 제공본은 같은 디자인의 1254×1254 별도 원본이며 byte hash가 다르다. Android의 canonical source는 위 사용자 제공본이다.

## 2. Android launcher 계약

- `DefaultLauncherAlias`, `CyberVaultLauncherAlias`, `DigitalRainLauncherAlias`, `PurpleNasLauncherAlias` 네 alias가 각각 위 순서의 icon을 가리킨다. 기존 설치 호환을 위해 Vibe Coder는 기존 `DigitalRainLauncherAlias` class name을 유지한다.
...
- API 26 이상은 adaptive wrapper가 원본 bitmap을 foreground로 직접 참조한다. API 33 이상은 별도 단색 NAS glyph를 `monochrome`으로 제공해 themed icon에서도 빈 아이콘이 되지 않게 한다.
...
- 밝은/어두운/고대비와 themed icon에서 foreground/background 대비를 확인한다. PNG가 opaque이므로 adaptive background는 parallax edge 보완용일 뿐 원본 색을 덮어쓰지 않는다.

## 5. 회귀 검증

1. JVM asset test가 네 `drawable-nodpi` 파일의 SHA-256을 위 값과 대조한다.
2. icon policy test가 네 variant↔alias가 1:1이고 MainActivity가 managed set에 없음을 확인한다.
3. merged manifest에서 application/default/세 alternate alias의 icon·roundIcon, enabled 기본값과 launcher filter를 확인한다.
4. API 26/32/33+에서 각 icon 전환 후 launcher와 app drawer의 실제 icon을 확인하고 재부팅·앱 업데이트 뒤 선택 복원을 확인한다.
5. API 33+ themed icon on/off에서 monochrome glyph가 표시되고 선택 alias가 유지되는지 확인한다.
